...
const sketchLayerToMixed = (layer, parent, nest = true, parentLayer = null) => {
    ...
    if (layer.name == 'text') {
        ...
        if ((resizingConstraint & FLAG_C) == 0)
            parent.append({
                prop: 'padding-left',
                value: convUnit(layer.frame.x - parentLayer.frame.x)
            });
        if ((resizingConstraint & FLAG_F) == 0)
            parent.append({
                prop: 'padding-top',
                value: convUnit(layer.frame.y - parentLayer.frame.y)
            });
    ...
    if (layer.name == 'container') {
        if ((resizingConstraint & FLAG_B) == 0)
            parent.append({
                prop: 'width',
                value: convUnit(layer.frame.width)
            });
        if ((resizingConstraint & FLAG_E) == 0)
            parent.append({
                prop: 'height',
                value: convUnit(layer.frame.height)
            });
        ...
        if (layer.frame.x !== 0)
            parent.append({
                prop: 'margin-left',
                value: convUnit(layer.frame.x)
            });
        if (layer.frame.y !== 0)
            parent.append({
                prop: 'margin-top',
                value: convUnit(layer.frame.y)
            });
        let marginRight =
    ...
    if (layer.style && layer.style.textStyle) {
        ...
        if (font !== '.SF NS Text')
            parent.append({
                prop: 'font-family',
                value: "'" + _.get(layer.style, 'textStyle.NSFont.family') + "'"
            });
        parent.append({
            prop: 'font-size',
            value: convUnit(
        ...
        parent.append({
            prop: 'color',
            value: convRGBA(layer.style.textStyle.NSColor.color)
        });
    }
