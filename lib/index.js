...
var processBorder = function processBorder(border, parent) {
  parent.append({
    prop: 'border-color',
  ...
  parent.append({
    prop: 'border-width',
    value: convUnit(_lodash2.default.get(border, 'thickness'))
  });
  parent.append({
    prop: 'border-style',
    value: 'solid'
...
var sketchLayerToMixed = function sketchLayerToMixed(layer, parent) {
  ...
  if (layer.name == 'text') {
    ...
    if ((resizingConstraint & FLAG_C) == 0) parent.append({ prop: 'padding-left', value: convUnit(layer.frame.x - parentLayer.frame.x) });
    if ((resizingConstraint & FLAG_F) == 0) parent.append({ prop: 'padding-top', value: convUnit(layer.frame.y - parentLayer.frame.y) });
  ...
  if (layer.name == 'container') {
    if ((resizingConstraint & FLAG_B) == 0) parent.append({ prop: 'width', value: convUnit(layer.frame.width) });
    if ((resizingConstraint & FLAG_E) == 0) parent.append({ prop: 'height', value: convUnit(layer.frame.height) });

    // Margins
    if (layer.frame.x !== 0) parent.append({ prop: 'margin-left', value: convUnit(layer.frame.x) });
    if (layer.frame.y !== 0) parent.append({ prop: 'margin-top', value: convUnit(layer.frame.y) });
    var marginRight = parentLayer.frame.width - layer.frame.x - layer.frame.width;
    if (marginRight !== 0) parent.append({ prop: 'margin-right', value: convUnit(marginRight) });
    var marginBottom = parentLayer.frame.height - layer.frame.y - layer.frame.height;
    if (marginBottom !== 0) parent.append({ prop: 'margin-bottom', value: convUnit(marginBottom) });
  }
  ...
  if (layer.style && layer.style.textStyle) {
    ...
    if (fontName.indexOf('italic')) parent.append({
      prop: 'font-style',
      value: 'italic'
    });
    if (fontName.indexOf('bold')) parent.append({
      prop: 'font-weight',
      value: 'bold'
    });
    ...
    parent.append({
      prop: 'font-size',
      value: convUnit(_lodash2.default.get(layer.style, 'textStyle.NSFont.attributes.NSFontSizeAttribute'))
    });
  ...
  if (layer.style && layer.style.borders.length && _lodash2.default.find(layer.style.borders, ['isEnabled', 1])) {
    processBorder(_lodash2.default.find(layer.style.borders, ['isEnabled', 1]), parent);
