...
exports.default = function (layerType, frame, parentFrame, resizingConstraint) {
    var rules = [];
    if (layerType === 'text') {
        if ((resizingConstraint & FLAG_C) === 0) rules.push({
            prop: 'padding-left',
            value: (0, _helpers.convUnit)(frame.x - parentFrame.x)
        });
        if ((resizingConstraint & FLAG_F) === 0) rules.push({
            prop: 'padding-top',
        ...
        if ((resizingConstraint & FLAG_D) === 0) {
            rules.push({
                prop: 'padding-bottom',
                value: (0, _helpers.convUnit)(parentFrame.y + parentFrame.height - frame.y - frame.height)
            });
        }
        if ((resizingConstraint & FLAG_A) === 0) {
            rules.push({
                prop: 'padding-right',
                value: (0, _helpers.convUnit)(parentFrame.x + parentFrame.width - frame.x - frame.width)
            });
        }
    }
    if (layerType == 'container') {
        if ((resizingConstraint & FLAG_B) === 0) rules.push({
            prop: 'width',
            value: (0, _helpers.convUnit)(frame.width)
        });
        if ((resizingConstraint & FLAG_E) === 0) rules.push({
            prop: 'height',
            value: (0, _helpers.convUnit)(frame.height)
        });

        // Margins
        if (frame.x !== 0) rules.push({
            prop: 'margin-left',
            value: (0, _helpers.convUnit)(frame.x)
        });
        if (frame.y !== 0) rules.push({
            prop: 'margin-top',
            value: (0, _helpers.convUnit)(frame.y)
        });
        var marginRight = parentFrame.width - frame.x - frame.width;
        if (marginRight !== 0) rules.push({
            prop: 'margin-right',
            value: (0, _helpers.convUnit)(marginRight)
        });
        if ((resizingConstraint & FLAG_D) === 0) {
            var marginBottom = parentFrame.height - frame.y - frame.height;
            if (marginBottom !== 0) rules.push({
                prop: 'margin-bottom',
