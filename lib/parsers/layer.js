...
var sketchLayerToMixed = function sketchLayerToMixed(layer, parent, opts) {
    var nest = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    var parentLayer = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
    ...
    if (layer.hasBackgroundColor && layer.includeBackgroundColorInExport) {
        ...
        if (layer.style && layer.style.fills.length && _lodash2.default.find(layer.style.fills, ['isEnabled', 1])) {
            (0, _helpers.appendRules)(parent, parser.fill(_lodash2.default.find(layer.style.fills, ['isEnabled', 1])));
        }
    }

    if (layer.layers) {
        layer.layers.forEach(function (childLayer) {
            if (nest && ['.', ':'].indexOf(childLayer.name.substring(0, 1)) !== -1) {
                var newParent = parent.cloneBefore();
                newParent.removeAll();
                var childName = childLayer.name;
                if (childName.substring(0, 1) !== ':') {
                    if (opts.cssModulesMode) newParent.selector += ' :global(' + childName + ')';else newParent.selector += ' ' + childName;
                } else {
            ...
            if (childLayer.name == 'container') sketchLayerToMixed(childLayer, parent, opts, nest, layer);
            if (childLayer.name == 'text') sketchLayerToMixed(childLayer, parent, opts, nest, boundingLayer);
        });
    }

    // Padding & Margins
    if (layer.frame && parentLayer) {
        (0, _helpers.appendRules)(parent, parser.frame(layer.name, layer.frame, parentLayer.frame, layer.resizingConstraint));
    }
    ...
    if (layer.style && layer.style.borders.length && _lodash2.default.find(layer.style.borders, ['isEnabled', 1])) {
        (0, _helpers.appendRules)(parent, parser.border(_lodash2.default.find(layer.style.borders, ['isEnabled', 1])));
    }
    ...
    // Do the box shadow...
    if (layer.style && _lodash2.default.find(layer.style.shadows, ['isEnabled', 1])) {
